...
distribution.
*/
// changes in this version: some formal modifications for gcc -Weffc++ compiler option marked "-mn"
...
class TINYXML2_LIB XMLDocument : public XMLNode {
   ...
   private:
    ...
    class DepthTracker {
       public:
        explicit DepthTracker(XMLDocument* document) : _document(document) {  // -mn: -Weffc++ fix
            document->PushDepth();
        }
        ~DepthTracker() {
            _document->PopDepth();
        }
        DepthTracker(const DepthTracker&) = delete;                          // -mn: -Weffc++ fix
        int operator=(const tinyxml2::XMLDocument::DepthTracker&) = delete;  // -mn: -Weffc++ fix
